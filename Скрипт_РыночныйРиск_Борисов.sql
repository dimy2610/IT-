...
/* Пункт 2.
Первым запросом считаем долю в процентах пустых строк в столбце [HaveDefault].*/
SELECT COUNT(*)*100/(SELECT COUNT(*) FROM [HW].[dbo].[bond_discription]) FROM [HW].[dbo].[bond_discription] WHERE [HaveDefault]=' ';
GO

 /*Получили 92%. Значит в столбце меньше, чем в 10% ячеек есть значения.
Столбец нужно убрать.
Следующим запросом сохраняем значения непустых строк в столбце [HaveDefault] в новую таблицу,
чтобы в случае необходимости данную информацию было можно найти.*/
SELECT [ISIN, RegCode, NRDCode], [HaveDefault] INTO [HW].[dbo].[bond_discription_HaveDefault] FROM [HW].[dbo].[bond_discription] WHERE [HaveDefault] !=' ';
GO

 /*После этого столбец можно удалить.*/
ALTER TABLE [HW].[dbo].[bond_discription] DROP COLUMN [HaveDefault];
GO
